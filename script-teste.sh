#!/bin/bash
gid=( $(aws rds describe-db-instances --query "*[].[VpcSecurityGroups]" |grep VpcSecurityGroupId |awk '{ print $2 }'|tr -d '"',',') )
list1=( $(doctl compute droplet list | awk '{print $3}' |tr -d [:alpha:]|grep -Ev "^$" ) )
list2=( $(aws ec2 describe-security-groups \
    --group-id ${gid[1]} \
